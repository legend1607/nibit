...
class PointNetSetAbstractionMsg(nn.Module):
    def __init__(self, npoint, radius_list, nsample_list, in_channel, mlp_list, xyz_dim=4):
        ...
        for i in range(len(mlp_list)):
            convs = nn.ModuleList()
            bns = nn.ModuleList()
            last_channel = in_channel + self.xyz_dim
    ...
    def forward(self, xyz, points):
        ...
        B, N, C = xyz.shape
        assert C == self.xyz_dim, f"xyz dim mismatch: got {C}, expect {self.xyz_dim}"
        S = self.npoint
