...
def rotate_point_cloud_z(batch_data, return_rotmat=False):
    """ 
    Randomly rotate the point clouds to augment the dataset.
    Works for both 2D (BxNx2) and 3D (BxNx3).
    
    Input:
        batch_data: BxNxC array, original batch of point clouds (C=2 or 3)
    Return:
        BxNxC array, rotated batch of point clouds
    """
    B, N, C = batch_data.shape
    rotated_data = np.zeros_like(batch_data, dtype=np.float32)
    rot_mats = np.zeros((B, C, C), dtype=np.float32)
    ...
    for k in range(B):
        ...
        if C == 3:  # 3D 点云，绕 Z 轴旋转
            ...
            raise ValueError(f"Unsupported point dimension: {C}, must be 2 or 3")

        rotated_data[k, ...] = np.dot(batch_data[k, ...], rotation_matrix)
        rot_mats[k, ...] = rotation_matrix

    if return_rotmat:
        return rotated_data, rot_mats
    else:
        return rotated_data
