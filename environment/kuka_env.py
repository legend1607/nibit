...
class KukaEnv:
    ...
    - 支持 with 上下文管理：with KukaEnv(...) as env:
    ...
    def __init__(self, GUI: bool = False, kuka_file: str = "kuka_iiwa/model.urdf"):
        """
        :param GUI: 是否以 GUI 模式启动 PyBullet
        :param kuka_file: KUKA URDF 路径（相对于 pybullet_data 的路径）
        """
        self.GUI = GUI
        ...
        self.kuka_id: int = p.loadURDF(self.kuka_file, [0, 0, 0], [0, 0, 0, 1], useFixedBase=True)

        # 获取机械臂关节维度与范围
        self.config_dim: int = p.getNumJoints(self.kuka_id)
        # pose_range shape: (config_dim, 2)
        pr = []
        for j in range(self.config_dim):
            info = p.getJointInfo(self.kuka_id, j)
        ...
        self.kuka_end_effector_index: int = self.config_dim - 1
    ...
    def __str__(self) -> str:
        return f"KukaEnv({self.config_dim}D)"

    def __enter__(self) -> "KukaEnv":
        return self
    ...
    def set_config(self, joint_values: Sequence[float], robot_id: Optional[int] = None) -> None:
        ...
        :param robot_id: 可选，指定 robot body id（默认 self.kuka_id）
        """
        if robot_id is None:
            robot_id = self.kuka_id
        arr = np.asarray(joint_values, dtype=float).flatten()
        if arr.size != self.config_dim:
            raise ValueError(f"[KukaEnv] 状态维度不匹配: got {arr.size}, expected {self.config_dim}")
        # 裁剪范围
    ...
    def _point_in_free_space(self, state: Sequence[float]) -> bool:
        ...
        if s_arr.size != self.config_dim:
            raise ValueError(f"[KukaEnv] 状态维度不匹配: got {s_arr.size}, expected {self.config_dim}")

        # 将关节直接设置到该状态（resetJointState）
        for j in range(self.config_dim):
            p.resetJointState(self.kuka_id, j, float(s_arr[j]))
        ...
        if len(self.obstacles) > 0:
            ...
            for obs_id in self.obstacles:
                ...
                if obs_id in self.ignored_body_ids:
                    continue
                pts = p.getClosestPoints(self.kuka_id, obs_id, distance=0.0)
                if pts:
            ...
            contacts = p.getContactPoints(bodyA=self.kuka_id)
            ...
            for c in contacts:
                ...
                other = bodyB if bodyA == self.kuka_id else bodyA if bodyB == self.kuka_id else None
                if other is None:
                    # 未能确定另一方，保守认为碰撞
                    collision_found = True
                    break
    ...
    def get_end_effector_pos(self, config: Sequence[float]) -> np.ndarray:
        ...
        pos = p.getLinkState(self.kuka_id, self.kuka_end_effector_index)[0]
    ...
    def render_path(
        ...
        """
        在 PyBullet GUI 中渲染一条路径（关节路径 -> 末端轨迹）
        ----------------------------------------------------------
        :param path: 关节空间路径（列表，每个元素为一个配置向量）
        :param color: RGB 颜色，例如 (1, 0, 0)
        :param line_width: 线条宽度
        :param life_time: 显示持续时间（0 表示永久）
        :param clear_previous: 是否清除旧的 debug line
        :param show_nodes: 是否在节点处添加小点
        """
        if not path or len(path) < 2:
            print("[KukaEnv] render_path(): path too short, skipping.")
            return
        ...
        pts = []
        for q in path:
            pos = self.get_end_effector_pos(q)
            pts.append(pos)
        ...
        if show_nodes:
            for pos in pts:
                ...
                p.addUserDebugLine(
                    [pos[0], pos[1], pos[2]],
                    [pos[0], pos[1], pos[2] + 0.01],
                    lineColorRGB=(color[0], color[1], color[2]),
