...
def interpolate_path(path: Sequence[Sequence[float]], step_size: float = 0.02):
    """
...
class UR5Env:
    ...
    - 支持 with 上下文管理：with UR5Env(...) as env:
    ...
    def __init__(self, GUI: bool = False, arm_file: str = "ur5/ur5.urdf"):
        """
        :param GUI: 是否以 GUI 模式启动 PyBullet
        :param arm_file: UR5 URDF 路径（相对于 pybullet_data 的路径）
        """
        self.GUI = GUI
    ...
    def __str__(self) -> str:
        return f"UR5Env({self.config_dim}D)"

    def __enter__(self) -> "UR5Env":
        return self
    ...
    def set_config(self, joint_values: Sequence[float], robot_id: Optional[int] = None) -> None:
        ...
        if arr.size != self.config_dim:
            raise ValueError(f"[UR5Env] 状态维度不匹配: got {arr.size}, expected {self.config_dim}")
        # 裁剪范围
    ...
    def _point_in_free_space(self, state: Sequence[float]) -> bool:
        ...
        if s_arr.size != self.config_dim:
            raise ValueError(f"[UR5Env] 状态维度不匹配: got {s_arr.size}, expected {self.config_dim}")
    ...
    def render_path(
        ...
        if not path or len(path) < 2:
            print("[UR5Env] render_path(): path too short, skipping.")
            return
